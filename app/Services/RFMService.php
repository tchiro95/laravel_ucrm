...
class RfmService
{
    public static function rfm($subQuery, $rfmPrms)
    {
        ...
        //プレースホルダーの「？」は第二引数の値が入る。配列だと？？？の一つ目、二つ目、と順番に入っていく。
        // 5.ランク毎の数を計算する
        ...
        $rCount = DB::table($subQuery)
            ...
            ->groupBy('rank')
            ->selectRaw('rank as r, count(r)')->orderBy('r', 'desc')->pluck('count(r)');
        //pluckはカウントrの数を抽出
        // ->get();
        ...
        $fCount = DB::table($subQuery)
            ...
            ->groupBy('rank')
            ->selectRaw('rank as f, count(f)')->orderBy('f', 'desc')->pluck('count(f)');
        // ->get();
        $mCount = DB::table($subQuery)
            ->rightJoin('ranks', 'ranks.rank', '=', 'm')
            ->groupBy('rank')
            ->selectRaw('rank as m, count(f)')->groupBy('m')
            ->selectRaw('m, count(m)')->orderBy('m', 'desc')->pluck('count(m)');
        // ->get();
        ...
        $data = DB::table($subQuery)
            ->rightJoin('ranks', 'ranks.rank', '=', 'r')
            ->groupBy('rank')
            ->selectRaw('concat("r_", rank) as rRank, count(case when f = 5 then 1 end ) as f_5, count(case when f = 4 then 1 end ) as f_4, count(case when f = 3 then 1 end ) as f_3, count(case when f = 2 then 1 end ) as f_2, count(case when f = 1 then 1 end ) as f_1')->orderBy('rRank', 'desc')
            ->get();
